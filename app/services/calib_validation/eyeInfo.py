    ...
    def init_points(self):
        try:
            ...
            if self.is_right:
                ...
                self.prediction_df = data[[
                    'screen_x', 'screen_y', 'right_iris_x', 'right_iris_y', 'left_iris_x', 'left_iris_y']]
            ...
            if is_subset:
                for j in range(len(self.calib_points)):
                    ...
                    if not eyes_only:
                        ...
                        sns.scatterplot(data=sub_calib_df, x='screen_x',
                                        y='screen_y', marker='*', color=self.palette[j], s=300)
                        if display_centroid:
                            sns.scatterplot(data=centroid_df, x='x', y='y', markers='x',
                                            s=max_distance, color=self.palette[j], alpha=0.5)
                            circle = plt.Circle(
                                (centroid_x, centroid_y), max_distance, color=self.palette[j], fill=False)
                            plt.gca().add_patch(circle)

                    sns.scatterplot(
                        data=subset_df, x=keys_x[i], y=keys_y[i], color=self.palette[j], ax=ax, s=10)
            else:
                sns.scatterplot(
                    data=datasets[i], x=f'{keys_x[i]}', y=f'{keys_y[i]}', color=colors[i], ax=ax)
        if lock_plot:
